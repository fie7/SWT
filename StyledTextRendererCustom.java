 ...
 */
class StyledTextRendererCustom {
    Device device;
    StyledTextCustom styledText;
...
StyledTextRendererCustom(Device device, StyledTextCustom styledText) {
    this.device = device;
...
void calculate(int startLine, int lineCount) {
    if (startLine % 10000 == 0) {
        System.out.println("calculate! startLine = " + startLine + ", lineCount = " + lineCount + "!!!!");
    ...
    for (int i = startLine; i < endLine; i++) {
        ...
        if (line.needsRecalculateSize()) {
            ...
            if (startLine % 10000 == 0) {
                System.out.println(i + ": recalculate size!!!! (startLine: " + startLine + ", endLine: " + endLine + "). duration: " +  Util.humanReadableFormat(Duration.between(start, Instant.now())));
            }
        }
        if (line.width > maxWidth) {
            ...
            maxWidthLineIndex = i;
            if (startLine % 10000 == 0) {
                System.out.println(i + ": AAAAAAAA!!!! (startLine: " + startLine + ", endLine: " + endLine + ")");
            }
        }
    }
}
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            for (i = 0; i < lineCount; i++) {
                LineSizeInfo line = getLineSize(i);
                if (line.needsRecalculateSize()) {
                    //System.out.println("calculateIdle -> i = " + i);
                    calculate(i, 1);
...
void copyInto(StyledTextRendererCustom renderer) {
    if (ranges != null) {
...
int getHeight () {
    ...
    if (styledText.isFixedLineHeight()) {
        System.out.println("fixed line height!!!!!!!!!!!!!!!!!!!");
    ...
    int retVal = totalHeight + styledText.topMargin + styledText.bottomMargin;
    System.out.println("TOTAL HEIGHT: " + retVal);
    return retVal;
}
