 ...
 */
public class StyledTextCustom extends Canvas {
    ...
    static final Comparator<Point> SELECTION_COMPARATOR = Comparator.comparingInt(StyledTextCustom::getX).thenComparingInt(p -> p.y);
    ...
    StyledTextRendererCustom renderer;
    Listener listener;
    ...
    MouseNavigatorCustom mouseNavigator;
    ...
    static class Printing implements Runnable {
        ...
        Printer printer;
        StyledTextRendererCustom printerRenderer;
     ...
     */
    Printing(StyledTextCustom styledText, Printer printer, StyledTextPrintOptions printOptions) {
        ...
        printerRenderer = new StyledTextRendererCustom(printer, null);
     ...
     *  line data for.
     */
    void cacheLineData(StyledTextCustom styledText) {
        StyledTextRendererCustom renderer = styledText.renderer;
 ...
 * @see #getStyle
 */
public StyledTextCustom(Composite parent, int style) {
    ...
    renderer = new StyledTextRendererCustom(getDisplay(), this);
    ...
    setData("DEFAULT_DROP_TARGET_EFFECT", new StyledTextDropTargetEffectCustom(this));
...
public int getLinePixel(int lineIndex) {
    ...
    if (lineIndex > topIndex) {
        System.out.println("*********TOP INDEX: " + topIndex + ", LINE INDEX: " + lineIndex);
        Instant start = Instant.now();
        for (int i = topIndex; i < lineIndex; i++) {
            height += renderer.getLineHeight(i);
        }
        System.out.println("*************OUT OF LOOP (GETLINEHEIGHT)!!!! duration: " + Util.humanReadableFormat(Duration.between(start, Instant.now())));
    } else {
        System.out.println("TOP INDEX: " + topIndex + ", LINE INDEX: " + lineIndex);
        Instant start = Instant.now();
        for (int i = topIndex - 1; i >= lineIndex; i--) {
            //System.out.println("getLineHeight " + i + "!");
            height -= renderer.getLineHeight(i);
        }
        System.out.println("OUT OF LOOP (GETLINEHEIGHT)!!!! duration: " + Util.humanReadableFormat(Duration.between(start, Instant.now())));
    }
...
void handleVerticalScroll(Event event) {
    ...
    System.out.println("handle vertical scroll!");
}
/**
...
void initializeAccessible() {
    ...
    accTextExtendedAdapter = new AccessibleTextExtendedAdapter() {
        ...
        public void getCaretOffset(AccessibleTextEvent e) {
            e.offset = StyledTextCustom.this.getCaretOffset();
        }
        @Override
        public void setCaretOffset(AccessibleTextEvent e) {
            StyledTextCustom.this.setCaretOffset(e.offset);
            e.result = ACC.OK;
        }
        @Override
        public void getSelectionRange(AccessibleTextEvent e) {
            Point selection = StyledTextCustom.this.getSelectionRange();
            e.offset = selection.x;
            e.length = selection.y;
        }
        @Override
        public void addSelection(AccessibleTextEvent e) {
            StyledTextCustom st = StyledTextCustom.this;
        ...
        public void getCharacterCount(AccessibleTextEvent e) {
            e.count = StyledTextCustom.this.getCharCount();
        }
        @Override
    ...
    accControlAdapter = new AccessibleControlAdapter() {
        ...
        public void getValue(AccessibleControlEvent e) {
            e.result = StyledTextCustom.this.getText();
        }
    };
    ...
    addListener(SWT.Dispose, event -> {
        ...
        // cache obtained connection and query it even after StyledTextCustom
...
String getAssociatedLabel () {
    ...
    for (int i = 0; i < siblings.length; i++) {
        if (siblings [i] == StyledTextCustom.this) {
            if (i > 0) {
...
void setClipboardContent(int start, int length, int clipboardType) throws SWTError {
    ...
    if (clipboardType == DND.SELECTION_CLIPBOARD) {
        ...
        RTFWriterCustom rtfWriter = new RTFWriterCustom(this, start, length);
        String rtfText = getPlatformDelimitedText(rtfWriter);

        HTMLTransfer htmlTransfer = HTMLTransfer.getInstance();
        HTMLWriterCustom htmlWriter = new HTMLWriterCustom(this, start, length, content);
...
public void setMouseNavigatorEnabled(boolean enabled) {
    ...
    if (enabled) {
        mouseNavigator = new MouseNavigatorCustom(this);
    } else {
